        ...
        with open(f_events, 'rt') as f:
            ...
            for line in f:
                ...
                for cond in cond_names:
                    ...
                    weights_loss[val_loss].append(-1 * float(info[3])) # weights loss for trial_run1
...
def get_l2_analysis(subject_list, n_sub, contrast_list, method, exp_dir, result_dir, working_dir, output_dir): 
    ...
    # SelectFiles
    contrast_file = opj(output_dir, 'l1_analysis', '_subject_id_*', "con_00{contrast_id}.nii")
    ...
    threshold = MapNode(Analysis_Threshold(use_topo_fdr=False,
                                  use_fwe_correction=False,
                                  extent_threshold=10,
                                  height_threshold=0.001,
                                  height_threshold_type='p-value',
                                  force_activation = True), name = "threshold", iterfield = ["stat_image", "contrast_index"])
...
def reorganize_results(result_dir, output_dir, n_sub, team_ID):
    ...
    h1 = opj(result_dir, output_dir, f"l2_analysis_equalIndifference_nsub_{n_sub}", '_contrast_id_02')
    h2 = opj(result_dir, output_dir, f"l2_analysis_equalRange_nsub_{n_sub}", '_contrast_id_02')
    h3 = opj(result_dir, output_dir, f"l2_analysis_equalIndifference_nsub_{n_sub}", '_contrast_id_02')
    h4 = opj(result_dir, output_dir, f"l2_analysis_equalRange_nsub_{n_sub}", '_contrast_id_02')
    h5 = opj(result_dir, output_dir, f"l2_analysis_equalIndifference_nsub_{n_sub}", '_contrast_id_04')
    h6 = opj(result_dir, output_dir, f"l2_analysis_equalRange_nsub_{n_sub}", '_contrast_id_04')
    h7 = opj(result_dir, output_dir, f"l2_analysis_equalIndifference_nsub_{n_sub}", '_contrast_id_04')
    h8 = opj(result_dir, output_dir, f"l2_analysis_equalRange_nsub_{n_sub}", '_contrast_id_04')
    h9 = opj(result_dir, output_dir, f"l2_analysis_groupComp_nsub_{n_sub}", '_contrast_id_03')
