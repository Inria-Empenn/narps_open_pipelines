...
""" Tests of the 'narps_open.pipelines.team_E3B6' module.

Launch this test with PyTest

Usage:
======
    pytest -q test_team_E3B6.py
    pytest -q test_team_E3B6.py -k <selected_test>
"""
...
from narps_open.pipelines.team_E3B6 import PipelineTeamE3B6

class TestPipelinesTeamE3B6:
    """ A class that contains all the unit tests for the PipelineTeamE3B6 class."""

    @staticmethod
    @mark.unit_test
    def test_create():
        """ Test the creation of a PipelineTeamE3B6 object """

        pipeline = PipelineTeamE3B6()

        # 1 - check the parameters
        assert pipeline.fwhm == 6.0
        assert pipeline.team_id == 'E3B6'
    ...
    @mark.unit_test
    def test_outputs():
        """ Test the expected outputs of a PipelineTeamE3B6 object """
        pipeline = PipelineTeamE3B6()
        # 1 - 1 subject outputs
    ...
    def test_parameters_files(temporary_data_dir):
        ...
        func_file = join(Configuration()['directories']['test_data'], 'pipelines',
            'team_E3B6', 'uasub-001_task-MGT_run-01_bold_resampled-32.nii')
        wc2_file = join(Configuration()['directories']['test_data'], 'pipelines',
            'team_E3B6', 'wc2sub-001_T1w-32.nii')
        reference_file = join(
            Configuration()['directories']['test_data'], 'pipelines',
            'team_E3B6', 'parameters_file.tsv')

        # Get new parameters file
        PipelineTeamE3B6.get_parameters_file(
    ...
    def test_subject_information():
        ...
        bunch = PipelineTeamE3B6.get_subject_information(test_file, 1)

        # Compare bunches to expected
        assert isinstance(bunch, Bunch)
        assert bunch.conditions == ['trial', 'button_press', 'no_response']
        ...
        assert pmod.name == ['param_loss', 'param_gain', 'expected_value']
        ...
        helpers.compare_float_2d_arrays(pmod.param, [
            ...
            [-4.11826547,  2.66470554,  1.45355992]
            ])
    ...
    @mark.pipeline_test
    def test_execution():
        """ Test the execution of a PipelineTeamE3B6 and compare results """
        helpers.test_pipeline_evaluation('E3B6')
