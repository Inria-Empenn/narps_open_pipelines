...
# CSIM: 3dttest with Clustsim opts for a single group
...
set grp_label      = group_analysis.CSIM.1grp.${grp}.${cond}
...
if ( -d /lscratch/$SLURM_JOBID ) then
    set usetemp  = 1
    set sdir_BW  = ${dir_csim}
    set dir_csim = /lscratch/$SLURM_JOBID #/${subj} #_${ses}
...
\cp -p ${template} ${dir_csim}/.
3dcopy -overwrite ${dir_grpqc}/${mask_name} ${dir_csim}/
...
if ( ${status} ) then
    echo "++ FAILED CSIM: ${grp_label}"
    set ecode = 1
    goto COPY_AND_EXIT
else
    echo "++ FINISHED CSIM: ${grp_label}"
endif
...
if( ${usetemp} && -d ${dir_csim} ) then
    echo "++ Used /lscratch"
    echo "++ Copy from: ${dir_csim}"
    echo "          to: ${sdir_BW}"
    \mkdir -p ${sdir_BW}
    \cp -pr   ${dir_csim}/* ${sdir_BW}/.
...
if ( ${ecode} ) then
    echo "++ BAD FINISH: CSIM (ecode = ${ecode})"
else
    echo "++ GOOD FINISH: CSIM"
endif
